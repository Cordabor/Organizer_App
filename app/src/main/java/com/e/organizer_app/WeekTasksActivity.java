...
public class WeekTasksActivity extends AppCompatActivity {
    ...
    private static final int SHOW_TASK_REQUEST_CODE = 1001;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bottomNavigationView.setSelectedItemId(R.id.nav_week_tasks);
    ...
    public void setWeekDaysDates(){
        ...
        // Уменьшаем дату на индекс текущего дня недели, чтобы начать перебор с понедельника
        instance.add(Calendar.DAY_OF_MONTH,  - dayOfWeek);
    ...
    private void loadDayTasks(int dayNum, Date date){
        ...
        list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
                ...
                startShowTaskActivity(id);
            }
        });
    }

    public void startShowTaskActivity(long id){
        Intent intent = new Intent(this, ShowTaskActivity.class);

        Uri uri = Uri.parse(TasksProvider.CONTENT_URI + "/" + id);
        intent.putExtra(TasksProvider.CONTENT_ITEM_TYPE, uri);
        startActivityForResult(intent, SHOW_TASK_REQUEST_CODE);
    }

    @Override
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == SHOW_TASK_REQUEST_CODE && resultCode == RESULT_OK){
            setWeekDaysDates();
        }
    }
}
